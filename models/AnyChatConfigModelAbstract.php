...
abstract class AnyChatConfigModelAbstract
{
    ...
    public function getFormHelper()
    {
        if ($this->formHelper === null){
            AnyChatLoader::loadClass('AnyChatFormHelper');
            $this->formHelper = new AnyChatFormHelper($this);
        }
                                    ...
                                    if ($this->$method($v, $params)) {
                                        ...
                                        if ($addErrors) {
                                            if (isset($validator['message'])) {
                                                ...
                                                $this->addError($attr, sprintf(esc_html__('Incorrect "%s" value for "%s" language', 'anychat'), $this->getAttributeLabel($attr), $lang));
                                            }
                                        }
                                        $valid = false;
                                    }
    ...
    public function populate()
    {
        ...
        foreach ($this->getAttributes() as $attribute => $value) {
            ...
            if (isset($_POST[$name])) {
                if ($this->getAttributeType($attribute) == 'textarea') {
                    $attributes[$attribute] = sanitize_textarea_field($_POST[$name]);
                } else {
                    $attributes[$attribute] = sanitize_text_field($_POST[$name]);
                }
            }
        }
    ...
    {
        return AnyChatTools::isMultilang();
    }
    
    public function getLanguages()
    {
        return AnyChatTools::getLanguages();
    }
    
    public function getCurrentLanguage()
    {
        return AnyChatTools::getCurrentLanguage();
    }
    
    public function getDefaultLanguage()
    {
        return AnyChatTools::getDefaultLanguage();
    }
            ...
            if ($this->getAttributeType($attr) == 'switch') {
                $config['fields'][$name]['values'] = array(
                    array(
                        'id' => 'active_on',
                        'value' => true,
                        'label' => esc_html__('Enabled', 'anychat'),
                    ),
                    array(
                        'id' => 'active_off',
                        'value' => false,
                        'label' => esc_html__('Disabled', 'anychat'),
                    )
                );
            }
